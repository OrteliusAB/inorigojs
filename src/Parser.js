    ...
    if (relationIndex[currentIndex] !== undefined) {
      relationIndex[currentIndex].forEach(childIndex => {
        if (row.cells[childIndex] !== undefined) {
          let attrName = objectIndex.filter(objIndex => { return objIndex.start === childIndex })[0].title;
          let objValue = rowObjects[childIndex].filter(childObj => { return childObj.ID === row.cells[childIndex] });
          if (currentObject[attrName] === undefined) {
            currentObject[attrName] = [];
          }
          if (currentObject[attrName].filter(item => { return item.ID === objValue[0].ID; }).length === 0 && objValue[0].ID !== null) {
